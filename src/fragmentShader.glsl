...
vec2 cellular(vec3 P) {
	...
	#define jitter 0.5 // smaller jitter gives more regular pattern
...
void main() {
	...
	float c = circle(circlePos, .04, 3.);

	float offx = v_uv.x + sin(v_uv.y + u_time * .1);
  	float offy = v_uv.y - u_time * 0.1 - cos(u_time * .001) * .01;

	vec2 F = cellular(vec3(st * vec2(24), u_time * 0.1) * 4.0) - 0.5;
	//vec2 F = cellular(vec3(offx, offy, u_time * 0.1) * 4.0) - 1.0;
	float dots = smoothstep(0.1, 0.0, F.x);
	float n = F.y-F.x;
	n *= dots;
